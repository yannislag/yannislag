...
void setup()
{
    ...
    /*
        ...
        if (dataReceived.px_pitch < 100) {
            dataReceived.px_pitch = 100;
        }
        if (dataReceived.px_pitch > 2500) {
            dataReceived.px_pitch = 2500;
        }

        if (dataReceived.px_roll < 100) {
            dataReceived.px_roll = 100;
        }
        if (dataReceived.px_roll > 2500) {
            dataReceived.px_roll = 2500;
        }

        if (dataReceived.px_yaw < 100) {
            dataReceived.px_yaw = 100;
        }
        if (dataReceived.px_yaw > 2500) {
            dataReceived.px_yaw = 2500;
        }
...
void loop()
{
    ...
    // roll = 0;
    // yaw = 0;
    pitch_error = (ypr[1] - (float)pitch);
    pitch_PID = pitch_kp * pitch_error;

    roll_error = (ypr[2] - (float)roll);
    roll_PID = roll_kp * roll_error;

    yaw_error = (ypr[0] - (float)yaw);
    yaw_PID = yaw_kp * yaw_error;

    // see the doc mpu_capteurs.docx page 6
    PWM_FR = throtle - (int)roll_PID - (int)pitch_PID - (int)yaw_PID; // motor 1
    PWM_RR = throtle - (int)roll_PID + (int)pitch_PID + (int)yaw_PID; // motor 2
    PWM_RL = throtle + (int)roll_PID + (int)pitch_PID - (int)yaw_PID; // motor 3
    PWM_FL = throtle + (int)roll_PID - (int)pitch_PID + (int)yaw_PID; // motor 4
